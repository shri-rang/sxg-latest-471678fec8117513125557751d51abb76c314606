...
class HomeSudentPage extends StatefulWidget {
  final List<String> studentInfo;

  const HomeSudentPage({Key key, this.studentInfo}) : super(key: key);

  @override
  _HomeSudentPageState createState() => _HomeSudentPageState();
}

class _HomeSudentPageState extends State<HomeSudentPage> {
  NetWorkAPiRepository _netWorkAPiRepository;

  @override
  void initState() {
    _netWorkAPiRepository = NetWorkAPiRepository();
    print(widget.studentInfo[0]);
    Future.delayed(Duration.zero, () {
      _showAlert(context);
    });
    // print(widget.parentInfo[1]);
    print(widget.studentInfo[2]);
    super.initState();
  }
      ...
      body: FutureBuilderWidget<List<StudentInfoModel>>(
          ...
          builder: (context, snapshot) {
            if (snapshot == null)
              return Center(
                child:
                    Text("No internet connection"), //distroy karo session yeha
              );
            return MaterialApp(
